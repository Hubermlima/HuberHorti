...
const prompt = `Você é um analista de Bitcoin. Analise os históricos abaixo e dê um diagnóstico objetivo e conciso (no máximo em 15 linhas) do que está acontecendo e uma conclusão clara. Linguagem simples, sem jargão.\n\nPreço: $${preco} (${precoDirecao}, ${varPct}% no dia)\nHistórico preços: ${JSON.stringify(precoHist)}\nHistórico Funding Rate: ${JSON.stringify(fundingHist)}\nHistórico Open Interest ($B): ${JSON.stringify(oiHist)}\nHistórico Top Trader Ratio: ${JSON.stringify(ratioHist)}\n\nSem formatação, parágrafo corrido.`;
