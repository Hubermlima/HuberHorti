...
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.operador_confirmou = true AND OLD.operador_confirmou IS DISTINCT FROM NEW.operador_confirmou THEN
...
FOR EACH ROW
WHEN (NEW.operador_confirmou = true AND OLD.operador_confirmou IS DISTINCT FROM NEW.operador_confirmou)
